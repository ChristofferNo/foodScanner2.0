...
const TestPage = () => {
  ...
  // Hantera när ett alternativ väljs
  const handleOptionSelect = (option, value) => {
    // Uppdatera det valda alternativet för den sektionen
    setSelectedOptions((prev) => ({
      ...prev,
      [option]: value,
    }));

    // När ett alternativ väljs, stäng dropdownen för den sektionen
    setVisibleOptions((prev) => ({
      ...prev,
      [option]: false,
    }));
      ...
      {visibleOptions.people && (
        <View style={styles.optionsList}>
          {people.map((person) => (
            ...
            >
              <Text  style={styles.selectableOptions}>{person.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
